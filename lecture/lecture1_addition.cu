...
__host__ __device__ float f(float a, float b){ // this dude has device, host so that I can call both cpu and gpu side
  return a+b;
}
...
__global__ void vecAddKernel(float *x,float *y,float *z,std::size_t N){  
  ...
  if(i<N)
    z[i]=f(x[i],y[i]); // Now every thread launched by this code willl execute this line with the index i given as such.
...
int main(int argc, char **argv){
  ...
  vecAddGpu(x,y,z,N);
  std::cout << "Time elapsed: " << t2.elapsed() << " seconds for GPU computation\n";
